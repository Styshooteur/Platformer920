...
{
    if (!buffer_.loadFromFile("data/sound/Jump_Sound.wav"))
    {
        std::cout << "Could not load jump sound ! \n";
    }
    sound_.setVolume(30);
    sound_.setBuffer(buffer_);

    if(!idleTexture_.loadFromFile("data/sprites/hero/still/sprite_idle.png"))
    {
    ...
    for(int i = 0; i < 3; i++)
    {
        ...
        oss << "data/sprites/hero/move/hero" << (i + 1) << ".png";
        walkTexture_[i].loadFromFile(oss.str());
    }
    jumpTexture_.loadFromFile("data/sprites/hero/jump/sprite_jump03.png");   
    ...
    b2Vec2 boxSize;
    boxSize.Set(
        80.0f / game_.pixelToMeter / 2.0f, //* playerScale_, 
        160.0f / game_.pixelToMeter / 2.0f); //* playerScale_);
